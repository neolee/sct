**SCT - Squirrel Configuration Tool**

**核心理念：尊重 Rime 逻辑，简化用户操作**

- **非破坏性**：SCT 永远不会修改 Rime 的默认配置文件（`default.yaml` 和 `squirrel.yaml`），所有更改都写入 `default.custom.yaml` 或 `squirrel.custom.yaml` 的 `patch` 键下。
